...
_mod.account.create('tabs', function (_this){
	var frame = _mod.account.frame;

	_this.select_tab = function(tab_id){
	...
	function _select_tab(state_id){
		...
		if ( 'undefined' != typeof state2tab[state_id] ) {
			...
			li.parents('.tab-ctrl').first().find('.d-tabs>div').eq(li.index()).fadeIn(500).siblings('div').hide();
		} else {
			$('.tab-ctrl div.tab').hide();
			$(state_id).fadeIn(500);
		}
